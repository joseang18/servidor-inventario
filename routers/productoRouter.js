...
router.post('/proveedores', async (req, res) => {
  try {
    ...
    if (!nombre || !nit) {
      return res.status(400).json({ success: false, error: 'El nombre de la empresa y el NIT są obligatorios.' });
    }
...
// --- NUEVAS RUTAS DE BÚSQUEDA PARA PRODUCTSEARCH ---

// 1. Buscar producto por código SKU exacto (Flexible: ignora mayúsculas y espacios)
...
// 2. Buscar coincidencias por nombre en la tabla productos (Estilo LIKE)
...
router.get('/generar-sku', async (req, res) => {
  try {
    console.log("Query recibido en /generar-sku:", req.query); // Permite ver qué variables exactas envía el frontend

    const { tipoProductoId, categoriaId, subcategoriaId, atributo } = req.query;
    
    // Captura flexible por si el frontend lo envía como 'nombre', 'nombreProducto' o 'q'
